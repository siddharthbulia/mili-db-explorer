...
export function applyAutoLimit(sql: string, limit: number): AutoLimitResult {
  if (!limit || limit <= 0) return { sql, injected: false };
  const trimmed = sql.trim();
  if (!trimmed) return { sql, injected: false };
  ...
  if (noTrail.includes(';')) return { sql, injected: false };
  if (!SELECT_RE.test(masked.trimStart())) return { sql, injected: false };
  if (LIMIT_RE.test(masked)) return { sql, injected: false };
  // SELECT INTO is a different beast (CREATE TABLE AS shorthand).
  if (INTO_RE.test(masked)) return { sql, injected: false };
