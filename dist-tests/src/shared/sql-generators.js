...
exports.generateCreateScript = generateCreateScript;
 ...
 */
function generateCreateScript(d) {
    ...
    lines.push(create);
    if (d.comment) {
        lines.push(`COMMENT ON TABLE ${target} IS ${quoteLit(d.comment)};`);
    }
    for (const c of d.columns) {
        if (c.comment) {
            lines.push(`COMMENT ON COLUMN ${target}.${quoteIdent(c.name)} IS ${quoteLit(c.comment)};`);
        }
    }
    for (const idx of d.indexes) {
        if (idx.isPrimary)
            continue; // already in PK
        lines.push(idx.definition + ';');
    }
    for (const fk of d.foreignKeys) {
        lines.push(`ALTER TABLE ${target} ADD CONSTRAINT ${quoteIdent(fk.name)} FOREIGN KEY (${fk.columns.map(quoteIdent).join(', ')}) REFERENCES ${quoteIdent(fk.refSchema)}.${quoteIdent(fk.refTable)} (${fk.refColumns.map(quoteIdent).join(', ')})${fk.onDelete && fk.onDelete !== 'NO ACTION' ? ' ON DELETE ' + fk.onDelete : ''}${fk.onUpdate && fk.onUpdate !== 'NO ACTION' ? ' ON UPDATE ' + fk.onUpdate : ''};`);
    }
    return lines.join('\n\n');
}
