...
async function bench(name, iters, fn, budgetP50, budgetP99) {
    const times = [];
...
async function main() {
    ...
    const conn = await (0, db_1.openConnection)(connect_1.TEST_CONNECTION);
    if (!conn.ok)
        throw new Error(conn.error);
    const results = [];
    // §14 reference numbers — adjusted for "local Postgres on a laptop".
    results.push(await bench('select 1 round-trip', 50, async () => { await (0, db_1.runQuery)(cid, 'select 1'); }, 50, 150));
    results.push(await bench('select 100 rows from 10-col table', 30, async () => { await (0, db_1.runQuery)(cid, 'select * from app.big_numbers limit 100'); }, 100, 300));
    results.push(await bench('select 5000 rows (buffered)', 10, async () => { await (0, db_1.runQuery)(cid, 'select n, squared from app.big_numbers'); }, 500, 1500));
    results.push(await bench('select 5000 rows (streamed, 1000/chunk)', 10, async () => {
        await (0, db_1.streamQuery)(cid, 'select n, squared from app.big_numbers', () => { }, { chunkSize: 1000 });
    }, 600, 1500));
    results.push(await bench('listSchemas (introspection)', 10, async () => { await (0, db_1.listSchemas)(cid); }, 600, 2000));
    results.push(await bench('refreshSchema with cached diff', 10, async () => { await (0, db_1.refreshSchema)(cid); }, 600, 2000));
    results.push(await bench('runQueryScript (3 statements)', 20, async () => { await (0, db_1.runQueryScript)(cid, 'select 1; select 2; select 3'); }, 100, 300));
    // Print table.
    process.stdout.write('\n');
    process.stdout.write(pad('case', 44) + pad('iters', 6) + pad('p50', 8) + pad('p95', 8) +
    ...
    for (const r of results) {
        if (!r.pass)
            failed++;
        process.stdout.write(pad(r.name, 44) +
